...
export default function Blackjack3D() {
    ...
    const [rageSignal, setRageSignal] = useState(0);
    ...
    return (
        <div className="relative h-screen w-screen overflow-hidden bg-[#05100b]">
            <Canvas shadows="percentage" dpr={[1, 2]} camera={{ position: CAMERA_POS, fov: CAMERA_FOV }} gl={{ antialias: true }}>
                {game.state && (
                    <Scene state={game.state} myId={user.id} selectedChip={selectedChip} premium={!!deck.premium} rageSignal={rageSignal} onSit={game.sit} onBet={game.bet} />
                )}
            </Canvas>
            ...
            {game.state ? (
                <Overlay
                    ...
                    deck={deck}
                    decks={decks}
                    onPickDeck={setDeckId}
                    onRage={() => setRageSignal((n) => n + 1)}
                    onLeave={() => router.push('/blackjack')}
                />
            ) : (
